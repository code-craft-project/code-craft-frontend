...
export default function OrganizationProfile() {
    ...
    const { organization } = useContext(OrganizationDashboardContext);
    ...
    return (
        <div className="w-full h-full flex flex-col py-5 px-20 bg-gray-950 rounded-xl">
            ...
            <div className="w-2/3 flex items-center justify-between mb-4">
                ...
                <input className={`text-gray-50  font-medium w-4/6 duration-300 border-none outline-none focus:bg-gray-800 ${isEditMode ? "bg-gray-900 px-4 py-2 rounded-lg" : "bg-transparent"}`} value={organization.name} disabled={!isEditMode} />
            </div>
            <div className="w-2/3 flex items-center justify-between mb-4">
                <span className="text-sm font-semibold text-gray-600 w-1/6">Type:</span>
                <input className={`text-gray-50  font-medium w-4/6 duration-300 border-none outline-none focus:bg-gray-800 ${isEditMode ? "bg-gray-900 px-4 py-2 rounded-lg" : "bg-transparent"}`} value={organization.type} disabled={!isEditMode} />
            </div>
...

            {
                isEditMode ? (
                    <div className='w-full flex items-center justify-end my-4'>
                        <div onClick={() => setIsEditMode(false)} className='w-fit self-end border border-px border-yellow-600 px-8 py-2 rounded-lg font-semibold text-yellow-600 mr-4 cursor-pointer'>Cancel</div>
                        <div className='w-fit self-end bg-yellow-600 border border-px border-yellow-600 px-8 py-2 rounded-lg font-semibold text-gray-50'>Save</div>
                    </div>
                ) : (
